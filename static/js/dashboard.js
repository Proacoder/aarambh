...
async function loadDashboardData() {
  try {
    ...
    if (dashboardData.profile) {
      ...
      document.getElementById('dash-user-title').textContent = `${p.name}'s Navigator`;
      document.getElementById('dash-user-subtitle').textContent = `🎓 Stage: ${p.className || '10th'} | 📍 Home: ${p.district || 'Maharashtra'} | 💼 Category: ${p.category || 'General'}`;
      
      const authModeEl = document.getElementById('dash-auth-mode');
      if (authModeEl) {
        if (p.id && p.id.startsWith('google')) {
          authModeEl.innerHTML = '🟢 <span>Google Account</span>';
        } else {
          authModeEl.innerHTML = '👤 <span>Guest Mode</span> &bull; <a href="/login/google" style="color:var(--primary); font-weight:700; text-decoration:underline; margin-left:4px;">🔗 Connect Google</a>';
        }
      }

      // Dynamic Next Best Action customization
      const matches = dashboardData.matches || [];
      const nbaTitle = document.getElementById('nba-title');
      const nbaDesc = document.getElementById('nba-desc');
      const nbaBtn = document.getElementById('nba-cta-btn');
      const nbaProgress = document.getElementById('nba-progress-status');

      if (matches.length > 0) {
        const topMatch = matches[0];
        if (nbaTitle) nbaTitle.textContent = `Target Track: ${topMatch.name || 'Engineering & Technology'}`;
        if (nbaDesc) nbaDesc.textContent = `You have a ${topMatch.matchPct || 92}% match with this career. Dive into your personalized timeline.`;
        if (nbaBtn) {
          nbaBtn.href = '/roadmap';
          nbaBtn.innerHTML = '<span>Open Action Roadmap</span> <span>🗺️</span>';
        }
        if (nbaProgress) nbaProgress.textContent = 'Journey Progress: 85%';
      }
    }
